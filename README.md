...
- **Phase 9 — Companion UI library (v0.12.0 → v0.14.0)** 🧩 — importable primitives under `fitz_liveviews.ui.*` (dotted sub-path). **Cut 1** (v0.12.0): Pager / Toast / ConfirmDialog + `--flv-*` theme. **Cut 2** (v0.13.0): the 8 generic primitives — Button / Card / Badge / Alert / Input / Spinner / Icon / Modal. **9.C** (v0.14.0): the four bundled examples refactored onto the primitives, which surfaced + fixed two API gaps (`Button.submit`, `Input.required`/`clear`/`autocomplete`) — see [`docs/companion-ui-benefits.md`](docs/companion-ui-benefits.md).

See [ROADMAP.md](ROADMAP.md) for what is coming next — Phase 11.7 (client-side dynamic capabilities), the Admin ABM flagship showcase, and beyond.
